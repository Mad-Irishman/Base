package com.ss.config.jwt.filter;
...
public class PublicFilter extends OncePerRequestFilter {
    ...
    private String getAuthUri() {
        String authUrl = ConfJsTopt.getInstance().getApp().getKeycloakOpenidUrl() + "/auth";
        authUrl += "?response_type=" + RESPONSE_TYPE_CODE;
        authUrl += "&client_id=" + clientId;
        authUrl += "&state=" + generateState(30);
        authUrl += "&scope=" + SCOPE;
        authUrl += "&redirect_uri=" + ConfJsTopt.getInstance().getApp().getKeycloakClientUrl();
