...
import com.ss.topt.controller.api.ErrorDetailsDto;
...
public class ActionService {
    ...
    public Long createAction(String sessionId, String userId, String actionName) {
        ...
        try {
            ResponseEntity<String> response = restTemplate.exchange(ConfJsTopt.getInstance().getApp().getActionServiceUrlCreate(), HttpMethod.POST, requestEntity, String.class);
        ...
        String.format("Failed to create action, response status: %s, body: %s",
            ...
            throw new Except4SupportDocumented("ErrActionService006", "Error to create action",
                    String.format("Error to create action by url: '%s'", ConfJsTopt.getInstance().getApp().getActionServiceUrlCreate()), ex);
    ...
    private void closeActionProcess(Long actionId, Boolean isError) {
        ...
        try {
            ResponseEntity<String> response = restTemplate.exchange(
                    ConfJsTopt.getInstance().getApp().getActionServiceUrlClose(),
                    HttpMethod.PATCH,
                    requestEntity,
                    String.class);
            if (response.getStatusCode() != HttpStatus.OK) 
            ...
            throw new Except4SupportDocumented("ErrActionService013", "Error to close action",
                    String.format("Error to close action by url: '%s'", ConfJsTopt.getInstance().getApp().getActionServiceUrlClose()), ex);
        } catch (JsonProcessingException ex) {
